...
function ContentAbout() {
  ...
  const [payRespect, setShowRespect] = useState(false);
  ...
  return (
    <div className="about">
      ...
      {showFiabilite && (
        <p className={`${showFiabilite ? 'visible' : 'invisible'}`}>
      ...
      <h2 onClick={() => setShowRespect(!payRespect)}>
        Respect
        <img
          src={payRespect ? rollUp : unroll}
          alt="dérouler respect"
          className="arrowDown"
        />
      </h2>
      {payRespect && (
        <p className={`${payRespect ? 'visible' : 'invisible'}`}>
      ...
      {showService && (
        <p className={`${showService ? 'visible' : 'invisible'}`}>
      ...
      {showSecurite && (
        <p className={`${showSecurite ? 'visible' : 'invisible'}`}>
